class Hreshaker extends KendaniEak {
    ...
    bazmanal() {
        ...
        for (var i in hreshtak) {
            if (norVandak && norVandak[0] == Hreshaker[i] && norVandak[1] == [i].y) {
                var zuygiIndex = i;
                break;
            }
        }
        var norVandak = random(this.yntrelVandak(0, 1));
        if (zuygiIndex && norVandak && hreshaker[zuygiIndex].ser == this.zuyg && this.energy >= this.bazmanaluAragutyun)
            var norhreshaker = new Hreshaker(vandak[0], vandak[1], 2);
